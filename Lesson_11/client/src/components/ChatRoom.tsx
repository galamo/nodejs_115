...
import { initSocketConnection, onConnect, onConnectPromise } from "../socket-service";
...
const ChatRoom: React.FC<{ user: User; onLogout: () => void }> = ({
    ...
    useEffect(() => {
        ...
        // onConnect(newSocket, setHistoryLoaded, user)
        async function allFlow() {
            try {
                await onConnectPromise(newSocket, user)
